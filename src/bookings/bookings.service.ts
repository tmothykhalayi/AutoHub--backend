...
export class BookingService {
  ...
  async create(
    createBookingDto: CreateBookingDto & { userId: number },
  ): Promise<Booking> {
    const {
      userId,
      vehicleId,
    ...
    if (rentalDays > maxRentalDays) {
      throw new BadRequestException(
        `Maximum rental duration is ${maxRentalDays} days`,
      );
    }
    ...
    if (rentalDays < minRentalDays) {
      throw new BadRequestException(
        `Minimum rental duration is ${minRentalDays} day`,
      );
    }
    ...
    try {
      await this.mailService.sendBookingConfirmation(user.email, {
        ...
        dropoffLocation:
          dropoffLocation || vehicle.branch?.name || 'Main Branch',
        securityDeposit,
        rentalDays,
      } as any);
    ...
    this.logger.log(
      `Booking created - ID: ${savedBooking.id}, User: ${user.id}, Vehicle: ${vehicle.id}, Total: $${totalPrice}`,
    );
  ...
  async findAll(options?: {
    ...
    return bookings.map((booking) => {
      ...
      return {
        ...
        isCompleted,
        isCancellable,
        daysRemaining: isUpcoming
          ? Math.ceil(
              (booking.startDate.getTime() - today.getTime()) /
                (1000 * 60 * 60 * 24),
            )
          : 0,
      };
  ...
  async update(
    ...
    if (
      ...
      updateBookingDto.userId !== booking.user.id
    ) {
      const user = await this.userRepository.findOne({
    ...
    if (
      ...
      updateBookingDto.vehicleId !== booking.vehicle.id
    ) {
      ...
      if (conflict) {
        throw new ConflictException(
          'New vehicle is already booked for these dates',
        );
      }
    ...
    if (updateBookingDto.startDate || updateBookingDto.endDate) {
      ...
      if (conflict) {
        throw new ConflictException(
          'Vehicle is already booked for the new dates',
        );
      }
      ...
      if (datesChanged) {
        ...
        booking.totalPrice =
          (booking.vehicle.spec?.dailyRate || 50) * rentalDays;
    ...
    if (updateBookingDto.status) {
      ...
      if (
        ...
        throw new BadRequestException(
          `Invalid status transition from ${booking.status} to ${updateBookingDto.status}`,
        );
      }
    ...
    this.logger.log(
      `Booking updated - ID: ${id}, Changes: ${changes.join(', ')}`,
    );
  ...
  async remove(id: number): Promise<{ message: string }> {
    ...
    if (booking.status === 'active') {
      throw new BadRequestException(
        'Cannot delete active booking. Please cancel it first.',
      );
    }
    ...
    this.logger.log(
      `Booking deleted - ID: ${id}, User: ${user.id}, Vehicle: ${vehicle.id}`,
    );
  ...
  async getUserBookings(userId: number): Promise<Booking[]> {
    ...
    return bookings.map((booking) => ({
      ...
      isCancellable:
        today < booking.startDate && booking.status === 'confirmed',
    }));
  }
