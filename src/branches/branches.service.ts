...
export class BranchService {
  ...
  async create(
    ...
    if (!createBranchDto.branchCode) {
      createBranchDto.branchCode = await this.generateUniqueBranchCode(
    ...
    if (createBranchDto.latitude && createBranchDto.longitude) {
      const nearbyBranch = await this.checkNearbyBranches(
      ...
      if (nearbyBranch) {
        throw new ConflictException(
          `Another branch exists within 2km of this location: ${nearbyBranch.name}`,
        );
      }
    }
  ...
  private async checkNearbyBranches(
    latitude: number,
    longitude: number,
    ...
    const nearbyBranch = await this.branchRepository
      ...
      .where(
        `(6371 * acos(cos(radians(${latitude})) * cos(radians(branch.latitude)) * cos(radians(branch.longitude) - radians(${longitude})) + sin(radians(${latitude})) * sin(radians(branch.latitude)))) < :maxDistance`,
  ...
  private async validateBranchEmail(email: string): Promise<void> {
    ...
    if (existingBranch) {
      throw new ConflictException(
        `Email ${email} is already registered to branch "${existingBranch.name}"`,
      );
    }
  ...
  private async validateBranchPhone(phone: string): Promise<void> {
    ...
    if (existingBranch) {
      throw new ConflictException(
        `Phone ${phone} is already registered to branch "${existingBranch.name}"`,
      );
    }
  ...
  private async notifyBranchCreation(
    branch: Branch,
    currentUser?: User,
  ): Promise<void> {
    try {
      this.logger.log(
        `New branch created: ${branch.name} (${branch.branchCode})`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send branch creation notification: ${error.message}`,
      );
    }
  }
}
