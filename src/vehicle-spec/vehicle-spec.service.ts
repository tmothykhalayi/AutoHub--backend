...
export class VehicleSpecService {
  ...
  async findAll(options?: {
    ...
    if (options?.minSeats !== undefined || options?.maxSeats !== undefined) {
      ...
      if (options.minSeats !== undefined) {
        whereConditions.seats = { ...whereConditions.seats, $gte: options.minSeats };
      }
      if (options.maxSeats !== undefined) {
        whereConditions.seats = { ...whereConditions.seats, $lte: options.maxSeats };
      }
    }
  ...
  async update(id: number, updateDto: UpdateVehicleSpecDto, currentUser?: User): Promise<VehicleSpec> {
    ...
    if (spec.vehicles && spec.vehicles.length > 0) {
      const updatableFields = ['description', 'features', 'dailyRate', 'weeklyRate'];
    ...
    if (updateDto.make || updateDto.model || updateDto.year) {
      updatedData['category'] = this.determineVehicleCategory({ ...spec, ...updateDto });
    }
  ...
  async getAvailableCategories(): Promise<string[]> {
    ...
    const categories = [...new Set(specs.map(spec => spec.category).filter(Boolean))];
  ...
  private determineVehicleCategory(spec: CreateVehicleSpecDto | any): string {
    ...
    if (spec.make?.toLowerCase().includes('mercedes') || spec.make?.toLowerCase().includes('bmw')) return 'Premium';
    return 'Standard';
  }

  private async findDuplicateSpec(dto: UpdateVehicleSpecDto, excludeId: number): Promise<VehicleSpec | null> {
    const whereConditions: any[] = [];
  ...
  private calculateRentalSuitability(spec: VehicleSpec): string {
    ...
    if (spec.features && typeof spec.features === 'string' && (spec.features as string).includes('GPS')) {
  ...
  async getSpecStatistics(): Promise<any> {
    const allSpecs = await this.vehicleSpecRepository.find({ relations: ['vehicles'] });
    
    const totalSpecs = allSpecs.length;
    const totalVehicles = allSpecs.reduce((sum, spec) => sum + (spec.vehicles?.length || 0), 0);
